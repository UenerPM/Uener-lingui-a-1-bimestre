const authRepo = require('../repositories/authRepository-avap2');
...
const AuthController = {
  ...
  async login(req, res) {
    try {
      ...
      const user = await authRepo.validateCredentials(email.trim(), senha);
      ...
      req.session.user = {
        cpfpessoa: user.cpfpessoa,
        nomepessoa: user.nomepessoa,
        email: user.email,
