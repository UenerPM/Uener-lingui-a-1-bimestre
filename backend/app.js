...
// ===== ROTAS REFATORADAS =====
const apiRoutes = require('./src/routes/api.js');           // /api/produtos, /api/pix
const imagensRotas = require('./src/routes/imagens.js');     // /imgs/:filename
...
// Static files (frontend moved to ../frontend/public)
...
// ===== ROTAS REFATORADAS (NOVAS ARQUITETURA) =====
// Estas sobrescrevem as antigas se houver conflito
try {
  app.use('/api', apiRoutes);              // GET /api/produtos (com URLs de imagens)
  app.use('/imgs', imagensRotas);          // GET /imgs/:filename (streaming)
  console.log('[app] ✓ Rotas refatoradas carregadas');
} catch (err) {
  console.warn('[app] ⚠️  Erro ao carregar rotas refatoradas:', err.message);
}
