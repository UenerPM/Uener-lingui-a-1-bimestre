/* Script extraído de pagamento.html */
'use strict';
...
function log(...args) { console.log('[pagamento]', ...args); }
function logError(msg) { console.error('[pagamento] ❌', msg); }
function showError(msg) { showStatus(msg, 'erro'); }
...
async function init() {
  ...
  try {
    ...
    if (!user) { showStatus('Você precisa fazer login para acessar o pagamento', 'erro'); setTimeout(() => { window.location.href = '/login.html'; }, 1500); return; }
    const pedidoId = sessionStorage.getItem('idPedidoAtual');
    if (!pedidoId) { showStatus('Pedido não encontrado. Por favor, confirme um pedido antes de pagar.', 'erro'); setTimeout(() => { window.location.href = '/index.html'; }, 2000); return; }
    await carregarPedido(pedidoId);
    await carregarFormas();
    configurarListeners();
    log('Página carregada com sucesso');
  } catch (err) { log('Erro na inicialização:', err.message); showStatus(`Erro ao carregar página: ${err.message}`, 'erro'); }
}

async function carregarPedido(id) { /* ... lógica copiada do HTML original ... */
  const resp = await fetch(`/api/pedidos/${encodeURIComponent(id)}`, { credentials: 'include' });
  if (!resp.ok) throw new Error(`Erro HTTP ${resp.status}`);
  const json = await resp.json();
  if (!json.success) throw new Error(json.message || 'Resposta inválida');
...
function renderizarItens(itens) {
  ...
  if (!Array.isArray(itens) || itens.length === 0) { els.pedidoItens.innerHTML = '<p style="text-align: center; color: #999;">Nenhum item no pedido</p>'; return; }
  itens.forEach(item => {
